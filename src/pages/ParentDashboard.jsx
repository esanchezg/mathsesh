  ...
  useEffect(() => {
    async function load() {
      const [{ data: kp }, { data: sess }, { data: wf }] = await Promise.all([
        supabase.from('kid_profile').select('*, profiles(username)').single(),
        supabase.from('sessions').select('*').order('started_at', { ascending: false }).limit(30),
              ...
              <div className="space-y-2">
                {group.facts.map((f, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <span className="font-['Space_Mono'] text-white text-sm">
                      {f.operand_a} {group.symbol} {f.operand_b}
                    </span>
...
function SectionTitle({ children }) {
  return (
    <h3 className="font-['Barlow_Condensed'] font-black text-lg text-[#888] uppercase tracking-widest mb-2">{children}</h3>
  )
}
